...
def pr_title_error():
    print(
        """PR Title does not match the required format. Please ensure the PR title follows the format: 
        '<Something something ..> - IT Change #<number>'"""
    )
    sys.exit(1)
...
def get_pull_request_details(commit_hash, github_token, repo):
    """
    Retrieve pull request details for a given commit hash from a GitHub
    repository.
    ...
    Returns:
    A dictionary with pull request details or a message indicating
    ...
    if response.status_code == 200:
        ...
        return f"""Failed to retrieve data. HTTP Status Code:
            {response.status_code}. Error: {response.text}"""


def main():
    description = (
        "Get pull request details for a given commit hash from GitHub."
    )
    ...
    parser.add_argument(
        "--repo",
        required=True,
        help="Repository name with owner (e.g., octocat/Hello-World)",
    )
